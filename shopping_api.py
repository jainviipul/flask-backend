...
class ShoppingAssistantAPI:
    ...
    def serpapi_api_google_search(self,videos_info):
        ...
        for info in videos_info:
            ...
            prompt = f"""
                You are a specialized AI model designed for a single purpose: to extract or suggest electronic device names from a given YouTube video description. Your analysis and response must follow a strict set of rules.

                Core Logic:

                Your task is to analyze the provided text and determine if it refers to a specific, named electronic device (such as a smartphone, laptop, smartwatch, earbuds, tablet, camera, etc.) or a general category/price range of devices. First, try to extract the exact name of the product from the description. If no specific product name is found, then suggest some relevant options.

                Case 1: Specific Product Name Found
                If the description explicitly mentions a specific electronic product (e.g., “Samsung Galaxy S24 Ultra”, “MacBook Air M3”, “Sony WH-1000XM5”), your ONLY output should be that exact model name.
            ...
            phone_list = [line.strip() for line in response.strip().split('\n') if line.strip()]
            mobile_list.extend(phone_list)
